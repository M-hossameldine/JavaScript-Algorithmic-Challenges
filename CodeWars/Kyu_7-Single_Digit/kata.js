...
function singleDigit(n) {
  ...
  let binarySum = binary.match(/1/g).length; 
  return String(binarySum) > 1 ? singleDigit(binarySum) : binarySum;  
}
...
// Another Solution
function singleDigit(n) {
  while(n>9) n=Array.from(n.toString(2),Number).reduce((v,w) => v+w);
  return n;
}
